      ...
      <div class="flex flex-col py-10">
        ...
        <ul class="space-y-6 mt-4">
          <li v-for="(value, key) in $data.roster" :key="key">
            <div class="w-24 h-24 bg-gray-400 rounded-full overflow-hidden flex items-center justify-center">
              <p class="text-xs">{{ value.externalUserId.substr(0, 12) }}</p>
            </div>
            <p v-if="key === activeSpeakerId"> Currently speaking ... </p>
          </li>
        </ul>
      </div>
    </template>
...
export default {
  ...
  methods: {
    fetchOrCreateMeetingAndAttendee() {
      this.$http
        ._post(`/spaces/${this.$route.params.meetingTitle}/join`)
        .then(res => {
          console.log('the meeting response data', res);
    ...
    handleActiveSpeaker(attendeeIds) {
      if (attendeeIds.length) {
        const mostActiveSpeaker = attendeeIds[0];
        this.activeSpeakerId = mostActiveSpeaker;
      }
    },
